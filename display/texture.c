...
void	init_pixel_ray(t_cube *cube, int textnum)
{
	...
	cube->ray.texX = cube->mlx.wall_sprite[textnum].width - \
	cube->ray.texX - 1;
...
void	get_tex_color(t_cube *cube, int texnum, int x, int y)
{
	...
	cube->ray.texY = (int)cube->ray.tex_pos & \
	(cube->mlx.wall_sprite[texnum].height - 1);
	cube->ray.tex_pos += cube->ray.f_step;
	my_mlx_pixel_put(&cube->mlx.walls, x, y, \
	new_addr[cube->ray.texY * \
	cube->mlx.wall_sprite[texnum].height + cube->ray.texX]);
}
...
void	select_wall_to_put_pixel(t_cube *cube, int x)
{
	if (cube->ray.side == 1 && cube->ray.mapY < cube->player.yPos)
		...
		cube->ray.textnum = 3;
	else if (cube->ray.side == 0 && cube->ray.mapX > cube->player.xPos)
		cube->ray.textnum = 0;
	else
		cube->ray.textnum = 1;
}
